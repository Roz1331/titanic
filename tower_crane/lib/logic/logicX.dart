...
class LogicX {
  ...
  static void carriageXVelocity() {
    ...
    double sensorX = (WorldState.ropeLength * sin(_radianConverter(WorldState.windDirection)) * sin (_radianConverter(WorldState.windSpeed))).abs();
    ...
    for (int i = 0; i < sensorXList.length; i++) {
      for (int j = 0; j < rules[sensorXList[i]].length; j++) {
        velocityProbability[rules[sensorXList[i]][j]] =
        probabilities[i];
      }
    }
